...
export function auditCommand(options: AuditOptions = {}) {
  ...
  const lightResults = auditSemanticPairs(lightSemanticTokens as unknown as Record<string, string>);
  let lightPassed = 0;
  ...
  const darkResults = auditSemanticPairs(darkSemanticTokens as unknown as Record<string, string>);
  let darkPassed = 0;
  ...
  if (fs.existsSync(themeJsonPath)) {
    try {
      const customTheme = JSON.parse(fs.readFileSync(themeJsonPath, "utf-8"));
      if (customTheme.semanticTokens) {
        console.log(`  Found custom "theme.json" configuration. Running live APCA audit on user theme...`);
        const customResults = auditSemanticPairs(customTheme.semanticTokens);
